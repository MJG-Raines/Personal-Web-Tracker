...
import os
from backend.core.browsers.chromium_tracker import ChromiumTracker
from backend.core.activity_tracker import BrowserType, PlatformType

@pytest.fixture
def desktop_tracker():
    """Creates a desktop Chromium tracker"""
    tracker = ChromiumTracker(platform_type=PlatformType.DESKTOP.value, db_path="test_desktop.db")
    yield tracker
...
@pytest.fixture
def mobile_tracker():
    """Creates a mobile Chromium tracker"""
    tracker = ChromiumTracker(platform_type=PlatformType.MOBILE.value, db_path="test_mobile.db")
    yield tracker
...
def test_browser_type_initialization():
    ...
    desktop = ChromiumTracker(PlatformType.DESKTOP.value)
    assert desktop.browser_type == BrowserType.CHROMIUM_DESKTOP.value
    
    mobile = ChromiumTracker(PlatformType.MOBILE.value)
    assert mobile.browser_type == BrowserType.CHROMIUM_MOBILE.value
...
def test_handle_tab_activated(desktop_tracker, sample_tab):
    ...
    assert desktop_tracker.last_active['browser_type'] == BrowserType.CHROMIUM_DESKTOP.value
...
def test_mobile_specific_behavior(mobile_tracker, sample_tab):
    ...
    assert mobile_tracker.last_active['browser_type'] == BrowserType.CHROMIUM_MOBILE.value
